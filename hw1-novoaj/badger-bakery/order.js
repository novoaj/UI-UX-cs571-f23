...
function calculateSubtotal() {
	subtotal = 0
	for (i in ITEMS){
		if (document.getElementById(ITEMS[i].concat("-quantity")).value){
			price = document.getElementById(ITEMS[i].concat("-price")).innerText;
			quantity = document.getElementById(ITEMS[i].concat("-quantity")).value
...
function calculateSalesTax() {
	// grab dropdown text and check for button click
	state_dropdown = document.getElementById("state-tax")
	console.log(calculateSubtotal())
	// Document.getElementById("btn-what-is-my-sales-tax").addEventListener(onclick)
	return roundMoney(getSalesTaxRateForState(state_dropdown.value)*calculateSubtotal()); // TODO calculateSalesTax
}
